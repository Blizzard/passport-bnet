...
function Strategy(options, verify) {
  ...
  this._wowCharactersURL = options.charactersURL || 'https://' + getMasheryHost(options.region) + '/wow/user/characters';
  this._sc2ProfileURL = options.sc2ProfileURL || 'https://' + getMasheryHost(options.region) + '/sc2/profile/user';
}
...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  async.parallel({
    ...
    wowcharacters: function(done) {
      strategy.getWoWCharacters(accessToken, done);
    },
    sc2profile: function(done) {
      strategy.getSC2Profile(accessToken, done);
    }
...
Strategy.prototype.getId = function(accessToken, done) {
  this._oauth2.get(this._idURL, accessToken, function(err, body, res) {
    ...
    if (err) {
      done(new InternalOAuthError('Failed to fetch the user id', err));
    }

    try {
      json = JSON.parse(body);
    } catch (ex) {
      done(new Error('Failed to parse the user id'));
    }
...
Strategy.prototype.getWoWCharacters = function(accessToken, done) {
  this._oauth2.get(this._wowCharactersURL, accessToken, function(err, body, res) {
    var json;
    ...
    done(null, json.characters);
  });
};

Strategy.prototype.getSC2Profile = function(accessToken, done) {
  this._oauth2.get(this._sc2ProfileURL, accessToken, function(err, body, res) {
    var json;
