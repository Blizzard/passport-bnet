...
/** ====
  *  `getHost` | utility function -> Takes a two character region code and returns a Bnet host URL.
  *  Example -> getHost('eu'); // returns 'eu.battle.net'.
...
function getHost (region) {
  return (re === 'ch') ? 'www.battlenet.com.cn' : re + '.battle.net';
}

/** ====
 *  `Strategy` constructor | The Bnet authentication strategy authenticates requests by delegating to 
...
/* ====
    [Util] - Inherit from `OAuth2Strategy`.
==== */

util.inherits(Strategy, OAuth2Strategy);

/* ====
 * [Strategy userProfile function] - Retrieve user profile from Bnet.
...
Strategy.prototype.userProfile = function (accessToken, done) {
  this._oauth2.get(this._profileUrl, accessToken, function (err, body, res) {
    ...
    if (err) return done(new InternalOAuthError('Failed to fetch the user id', err));
...
/* ====
    [Exports] - Export our `Strategy` constructor.
==== */
